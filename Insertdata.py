...
if __name__ == '__main__':
    ...
    data_floor_tutor = pd.read_excel('floor_tutor.xlsx')

    #db.insert_into_table(data,column_indice(s),table_name,column_name(s))
    db.insert_into_table_tri(data,[16,9,10],'dormitory_supervisor',['Supervisor_ID','Name','Password'])
    
    db.insert_into_table_bi(data,[1,16],'dormitory',['Dormitory_ID','Dormitory_Supervisor_ID'])

    
    db.insert_into_table_tri(data_floor_tutor,[4,5,3],'floor_tutor',['Tutor_ID','Name','Password'])
    db.insert_into_table_tetra(data_floor_tutor,[0,1,2,4],'floor',['Dormitory_ID','Floor_Number','Gender','Tutor_ID'])
